...
root.title("AGE-CALCULATOR BY @invincible")   #setting up title
root.configure(bg="#D5C6FF")   #setting up backround color
root.geometry("600x500")    #fixing the size of the window
...
def age(b_date,b_month,b_year):
    ...
    new.config(font=("Arial Rounded MT Bold",25))
    new.grid(row=5,column=0,columnspan=3)
...
submit=Button(root,text="GET AGE!!",width=20,anchor=CENTER,command=lambda:age(b_date,b_month,b_year),bg="#6600CC",fg="#D5C6FF",borderwidth=5)
submit.grid(row=4,column=0)

#calling clean function in button widget
clear=Button(root,text="CLEAR",width=20,command=lambda:clean(entry_date,entry_month,entry_year),bg="#6600CC",fg="#D5C6FF",borderwidth=5)
